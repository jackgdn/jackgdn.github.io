...
document.addEventListener('keydown', function (event) {
    if (event.key === 'g') {
        ...
        if (gPressCount === 2) {
            ...
            gPressCount = 0;
        } else if (gPressCount === 1) {
            window.location.href = 'https://github.com/jackgdn/';
        }
    } else if (event.key === 'm') {
        window.location.href = 'mailto:1pk0@proton.me';
    } else if (event.key === 'd') {
        window.location.href = 'https://discordapp.com/users/1197744806026280980';
    }
});
...
document.addEventListener('DOMContentLoaded', (event) => {
    document.addEventListener('keydown', function (event) {
        if (event.key === '/') {
            event.preventDefault(); // 阻止默认的斜杠行为（如开始一个新行）
