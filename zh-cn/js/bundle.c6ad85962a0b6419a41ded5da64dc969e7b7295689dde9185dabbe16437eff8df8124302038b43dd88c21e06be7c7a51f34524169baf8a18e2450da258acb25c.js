...
document.addEventListener('keydown', function (event) {
    ...
    // Long press handlers
    if (!event.shiftKey && ['m', 'g', 'd'].includes(key) && !event.target.matches('input, textarea, [contenteditable]')) {
        longPressTimers[key] = setTimeout(() => {
            if (key === 'm') {
                window.open('mailto:1pk0@proton.me', '_blank');
            } else if (key === 'g') {
                window.open('https://github.com/jackgdn/', '_blank');
            } else if (key === 'd') {
                window.open('https://discordapp.com/users/1197744806026280980', '_blank');
            }
        }, 500);
    }

    // Double g press to top
    ...
    // Shift+g to bottom
    else if (key === 'g' && event.shiftKey && !event.target.matches('input, textarea, [contenteditable]')) {
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    }
    // Shift+n for theme toggle
    else if (key === 'n' && event.shiftKey && !event.target.matches('input, textarea, [contenteditable]')) {
